...
from haar_histogram import calc_histo, make_masks
from haar_utils import correct_image, detect_object, preprocessing
...
while True:
    ...
    if key == 2490368 or key == 65362:
        cnt = 1                   # 위쪽 화살표 키이면 다음 영상
        no = no + cnt
        if no >= max_no:
            no = 0
    elif key == 2621440 or key == 65364:
        cnt = -1                  # 아래쪽 화살표 키이면 이전 영상
        no = no + cnt
        if no < 0:
            no = max_no
